...
Harbr includes an on-device logging system that helps debug crashes, issues, and support requests. Harbr stores 4 different levels of logs: **Debug**, **Warning**, **Error**, and **Critical**.
...
{% tab title="Critical" %}
Critical logs are crashing errors or unhandled errors that have occurred on the device. Critical logs would occur when Harbr fails to complete the boot process, fails to render the UI, etc.
{% endtab %}
{% endtabs %}
...
Harbr also stores a list of the **last 100 logs** that have occurred on the device. To access the history of logs, go to Settings -> System -> Logs.

{% hint style="info" %}
The log database size is checked on startup, so all logs that have occurred in the active session will remain in the log history until Harbr is closed and reopened.
{% endhint %}

## Exporting Logs

Harbr offers the ability to export your logs into a JSON file, which can be easily sent to the developer to help debug the problems. The exported logs also include additional information, including the code-execution stack trace to see exactly where in the code the error occurred.
...
Harbr offers the ability to clear all recorded logs from your device. While an available option, it is not recommended to clear your logs often as Harbr will internally ensure the log database does not grow to an obscenely large size.
