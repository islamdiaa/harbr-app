...
class _State extends State<ConfigurationReadarrRoute>
  ...
  Widget _body() {
    return HarbrListView(
      ...
      children: [
        HarbrModule.READARR.informationBanner(),
        _enabledToggle(),
  ...
  Widget _enabledToggle() {
    return HarbrBox.profiles.listenableBuilder(
      builder: (context, _) => HarbrBlock(
        title: 'settings.EnableModule'.tr(args: [HarbrModule.READARR.title]),
        trailing: HarbrSwitch(
          value: HarbrProfile.current.readarrEnabled,
          onChanged: (value) {
            HarbrProfile.current.readarrEnabled = value;
      ...
      body: [
        TextSpan(
          text: 'settings.ConnectionDetailsDescription'.tr(
            args: [HarbrModule.READARR.title],
          ),
        )
      ],
  ...
  Widget _defaultOptionsPage() {
    return HarbrBlock(
      ...
      trailing: const HarbrIconButton.arrow(),
      onTap: SettingsRoutes.CONFIGURATION_READARR_DEFAULT_OPTIONS.go,
    );
  }

  Widget _queueSize() {
    const _db = ReadarrDatabase.QUEUE_PAGE_SIZE;
    return _db.listenableBuilder(
      builder: (context, _) => HarbrBlock(
        title: 'readarr.QueueSize'.tr(),
        body: [
          TextSpan(
            text: _db.read() == 1
                ? 'harbr.OneItem'.tr()
