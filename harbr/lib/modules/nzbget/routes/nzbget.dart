...
class _State extends State<NZBGetRoute> {
  ...
  NZBGetAPI _api = NZBGetAPI.from(HarbrProfile.current);
  ...
  void initState() {
    super.initState();
    _pageController =
        HarbrPageController(initialPage: NZBGetDatabase.NAVIGATION_INDEX.read());
  }
  ...
  Widget _drawer() => HarbrDrawer(page: HarbrModule.NZBGET.key);
  ...
  Widget _appBar() {
    List<String> profiles = HarbrBox.profiles.keys.fold([], (value, element) {
      if (HarbrBox.profiles.read(element)?.nzbgetEnabled ?? false)
        value.add(element);
      return value;
    });
    List<Widget>? actions;
  ...
  Widget _body() {
    if (!HarbrProfile.current.nzbgetEnabled)
      return HarbrMessage.moduleNotEnabled(
        context: context,
        module: HarbrModule.NZBGET.title,
      );
  ...
  Future<void> _sort() async {
    ...
    if (values[0])
      await _api.sortQueue(values[1]).then((_) {
        ...
        showHarbrErrorSnackBar(title: 'Failed to Sort Queue', error: error);
      });
  }
  ...
  void _refreshProfile() {
    _api = NZBGetAPI.from(HarbrProfile.current);
