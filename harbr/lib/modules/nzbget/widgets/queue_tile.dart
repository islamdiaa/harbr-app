...
class _State extends State<NZBGetQueueTile> {
  ...
  Widget build(BuildContext context) {
    return HarbrBlock(
      ...
      trailing: HarbrReorderableListViewDragger(index: widget.index),
      onTap: _handlePopup,
    );
  }
...
class _Helper {
  ...
  Future<void> _pauseJob() async {
    await NZBGetAPI.from(HarbrProfile.current).pauseSingleJob(data.id).then((_) {
      showHarbrSuccessSnackBar(title: 'Job Paused', message: data.name);
      refresh();
    }).catchError((error) {
  ...
  Future<void> _resumeJob() async {
    await NZBGetAPI.from(HarbrProfile.current)
        ...
        .then((_) {
      showHarbrSuccessSnackBar(title: 'Job Resumed', message: data.name);
      refresh();
    }).catchError((error) {
      showHarbrErrorSnackBar(
  ...
  Future<void> _category() async {
    List<NZBGetCategoryData> categories =
        await NZBGetAPI.from(HarbrProfile.current).getCategories();
  ...
  Future<void> _rename() async {
    ...
    if (values[0])
      NZBGetAPI.from(HarbrProfile.current)
          ...
          .then((_) {
        showHarbrSuccessSnackBar(title: 'Job Renamed', message: values[1]);
        refresh();
      }).catchError((error) {
        showHarbrErrorSnackBar(
  ...
  Future<void> _delete() async {
    ...
    if (values[0])
      await NZBGetAPI.from(HarbrProfile.current).deleteJob(data.id).then((_) {
        showHarbrSuccessSnackBar(title: 'Job Deleted', message: data.name);
        refresh();
      }).catchError((error) {
        showHarbrErrorSnackBar(
  ...
  Future<void> _password() async {
    ...
    if (values[0])
      await NZBGetAPI.from(HarbrProfile.current)
          ...
          .then((_) {
        showHarbrSuccessSnackBar(title: 'Job Password Set', message: data.name);
        refresh();
      }).catchError((error) {
        showHarbrErrorSnackBar(
