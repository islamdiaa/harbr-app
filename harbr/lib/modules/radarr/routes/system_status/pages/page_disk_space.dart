...
class _State extends State<RadarrSystemStatusDiskSpacePage>
      ...
      child: FutureBuilder(
        ...
        builder: (context, AsyncSnapshot<List> snapshot) {
          if (snapshot.hasError) {
            HarbrLogger().error('Unable to fetch Radarr disk space',
  ...
  Widget _list(
    ...
    // Compile Disks
    List<Widget> _disks = [
      HarbrMessage.inList(text: 'radarr.NoDisksFound'.tr())
    ];
    if (diskSpace.isNotEmpty)
      _disks = [
        HarbrHeader(text: 'radarr.Disks'.tr()),
        ...List.generate(
          diskSpace.length,
    ...
    List<Widget> _rootFolders = [
      HarbrMessage.inList(text: 'radarr.NoRootFoldersFound'.tr())
    ];
    if (rootFolders.isNotEmpty)
      _rootFolders = [
        HarbrHeader(text: 'radarr.RootFolders'.tr()),
        ...List.generate(
          rootFolders.length,
