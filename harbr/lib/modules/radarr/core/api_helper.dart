  ...
  Future<RadarrMovie?> addMovie({
    ...
    if (context.read<RadarrState>().enabled) {
      final result = await context
        ...
        if (showSnackbar)
          showHarbrErrorSnackBar(title: 'Failed to Add Movie', error: error);
        return RadarrMovie();
      });
  ...
  Future<bool> backupDatabase({
    ...
    if (context.read<RadarrState>().enabled) {
      return await context.read<RadarrState>().api!.command.backup().then((_) {
        if (showSnackbar)
          showHarbrSuccessSnackBar(
              title: 'Backing Up Database${HarbrUI.TEXT_ELLIPSIS}',
              message: 'Backing up the database in the background');
        return true;
      }).catchError((error, stack) {
        HarbrLogger().error('Unable to backup database', error, stack);
        if (showSnackbar)
          showHarbrErrorSnackBar(
  ...
  Future<bool> missingMovieSearch({
    ...
    if (context.read<RadarrState>().enabled) {
      return await context
        ...
        if (showSnackbar)
          showHarbrSuccessSnackBar(
              title: 'Searching${HarbrUI.TEXT_ELLIPSIS}',
  ...
  Future<bool> runRSSSync({
    ...
    if (context.read<RadarrState>().enabled) {
      return await context.read<RadarrState>().api!.command.rssSync().then((_) {
        if (showSnackbar)
          showHarbrSuccessSnackBar(
              title: 'Running RSS Sync${HarbrUI.TEXT_ELLIPSIS}',
              message: 'Running RSS sync in the background');
        return true;
      }).catchError((error, stack) {
        HarbrLogger().error('Unable to run RSS sync', error, stack);
        if (showSnackbar)
          showHarbrErrorSnackBar(title: 'Failed to Run RSS Sync', error: error);
        return false;
      });
    }
    return false;
  ...
  Future<bool> updateLibrary({
    ...
    if (context.read<RadarrState>().enabled) {
      return await context
        ...
        if (showSnackbar)
          showHarbrSuccessSnackBar(
              title: 'Updating Library${HarbrUI.TEXT_ELLIPSIS}',
              message: 'Updating library in the background');
        return true;
      }).catchError((error, stack) {
        HarbrLogger().error('Unable to update library', error, stack);
        if (showSnackbar)
          showHarbrErrorSnackBar(
  ...
  Future<bool> deleteMovieFile({
    ...
    if (context.read<RadarrState>().enabled) {
      return await context
        ...
        if (showSnackbar)
          showHarbrErrorSnackBar(title: 'Failed to Delete File', error: error);
        return false;
      });
    }
    return false;
  ...
  Future<bool> updateMovie({
    ...
    if (context.read<RadarrState>().enabled) {
      return await context
        ...
        HarbrLogger().error('Failed to update movie: ${movie.id}', error, stack);
        showHarbrErrorSnackBar(title: 'Failed to Update Movie', error: error);
        return false;
      });
    }
    return false;
  ...
  Future<bool> removeMovie({
    ...
    if (context.read<RadarrState>().enabled) {
      return await context
        ...
        HarbrLogger().error('Failed to remove movie: ${movie.id}', error, stack);
        showHarbrErrorSnackBar(title: 'Failed to Remove Movie', error: error);
        return false;
      });
    }
    return false;
  ...
  Future<bool> quickImport({
    ...
    if (context.read<RadarrState>().enabled) {
      return await context
        ...
        if (showSnackbar)
          showHarbrSuccessSnackBar(
            title: 'Running Quick Import${HarbrUI.TEXT_ELLIPSIS}',
            message: path,
          );
        return true;
      }).catchError((error, stack) {
        ...
        showHarbrErrorSnackBar(title: 'Failed to Quick Import', error: error);
        return false;
      });
    }
    return false;
  ...
  Future<bool> deleteTag({
    ...
    if (context.read<RadarrState>().enabled) {
      return await context
        ...
        HarbrLogger().error('Failed to add tag: ${tag.id}', error, stack);
        if (showSnackbar)
          showHarbrErrorSnackBar(title: 'Failed to Delete Tag', error: error);
        return false;
      });
    }
    return false;
  ...
  Future<bool> triggerManualImport({
    ...
    if (context.read<RadarrState>().enabled) {
      return await context
        ...
        HarbrLogger().error('Failed to trigger manual import', error, stack);
        if (showSnackbar)
          showHarbrErrorSnackBar(title: 'Failed to Import', error: error);
        return false;
      });
    }
    return false;
