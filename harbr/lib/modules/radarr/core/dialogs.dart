...
class RadarrDialogs {
  ...
  Future<Tuple2<bool, RadarrImportMode?>> setManualImportMode(
    ...
    await HarbrDialog.dialog(
      ...
      content: List.generate(
        ...
        (index) => HarbrDialog.tile(
          text: RadarrImportMode.values[index].harbrReadable,
          icon: RadarrImportMode.values[index].harbrIcon,
          iconColor: HarbrColours().byListIndex(index),
    ...
    await showDialog(
      ...
      builder: (_) => ChangeNotifierProvider.value(
        ...
        builder: (context, _) => AlertDialog(
          ...
          title: HarbrDialog.title(text: 'radarr.Languages'.tr()),
          content: Consumer<RadarrManualImportDetailsTileState>(
            builder: (context, manualImport, _) => HarbrDialog.content(
              children: List.generate(
                filteredLanguages.length,
                (index) => HarbrDialog.checkbox(
          ...
          builder: (context, future, _) => FutureBuilder(
            ...
            builder: (context, AsyncSnapshot<List<RadarrTag>> snapshot) {
              return AlertDialog(
                ...
                title: HarbrDialog.title(text: 'radarr.Tags'.tr()),
                content: Builder(
                  builder: (context) {
                    if (snapshot.data?.isEmpty ?? true)
                      return HarbrDialog.content(
                        children: [
                          HarbrDialog.textContent(
                ...
                contentPadding: (snapshot.data?.length ?? 0) == 0
                    ? HarbrDialog.textDialogContentPadding()
                    : HarbrDialog.listDialogContentPadding(),
                shape: HarbrUI.shapeBorder,
              );
            },
          ),
          ...
          builder: (context, future, _) => FutureBuilder(
            ...
            builder: (context, AsyncSnapshot<List<RadarrTag>> snapshot) {
              return AlertDialog(
                ...
                title: HarbrDialog.title(text: 'Tags'),
                content: Builder(
                  builder: (context) {
                    if ((snapshot.data?.length ?? 0) == 0)
                      return HarbrDialog.content(
                        children: [
                          HarbrDialog.textContent(text: 'No Tags Found'),
                        ],
                      );
                    return HarbrDialog.content(
                      children: List.generate(
  ...
  Future<bool> moveFiles() async {
    ...
    void _setValues(bool flag) {
      _flag = flag;
      Navigator.of(HarbrState.context, rootNavigator: true).pop();
    }
    ...
    await HarbrDialog.dialog(
      ...
      content: [
        HarbrDialog.textContent(text: 'radarr.MoveFilesDescription'.tr()),
      ],
    );
