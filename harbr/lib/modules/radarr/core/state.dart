...
class RadarrState extends HarbrModuleState {
  RadarrState() {
    reset();
  }
  ...
  HarbrListViewOption? _moviesViewType =
      RadarrDatabase.DEFAULT_VIEW_MOVIES.read();
  HarbrListViewOption get moviesViewType => _moviesViewType!;
  set moviesViewType(HarbrListViewOption moviesViewType) {
    ...
    if (_movies != null)
      _upcoming = _movies!.then((movies) {
        ...
        _missingOnly.forEach((movie) {
          if (movie.harbrIsInCinemas && !movie.harbrIsReleased)
            _notYetReleased.add(movie);
          if (!movie.harbrIsInCinemas && !movie.harbrIsReleased)
            _notYetInCinemas.add(movie);
        });
        _notYetReleased.sort((a, b) => a.harbrCompareToByReleaseDate(b));
        _notYetInCinemas.sort((a, b) => a.harbrCompareToByInCinemas(b));
    ...
    if (_movies != null)
      _missing = _movies!.then((movies) {
        List<RadarrMovie> _movies = movies.where((movie) {
          ...
          if (!movie.harbrIsReleased) return false;
          return true;
        }).toList();
        _movies.sort((a, b) {
          int? _comparison;
          if (a.harbrEarlierReleaseDate == null &&
              b.harbrEarlierReleaseDate != null) return 1;
          if (b.harbrEarlierReleaseDate == null &&
              a.harbrEarlierReleaseDate != null) return -1;
          if (a.harbrEarlierReleaseDate == null &&
              b.harbrEarlierReleaseDate == null) _comparison = 0;
          _comparison ??=
              b.harbrEarlierReleaseDate!.compareTo(a.harbrEarlierReleaseDate!);
          if (_comparison == 0)
            return a.sortTitle!
                .toLowerCase()
