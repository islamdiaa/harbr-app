...
class _State extends State<ScheduleView> {
  ...
  Widget build(BuildContext context) {
    ...
    return HarbrCustomScrollView(
      ...
      slivers: [
        const SliverPadding(
          padding: EdgeInsets.symmetric(vertical: HarbrUI.MARGIN_SIZE_HALF),
        ),
        ...schedule.item1,
        const SliverPadding(
          padding: EdgeInsets.only(bottom: HarbrUI.MARGIN_SIZE_HALF),
        ),
      ],
    );
  }
  ...
  Tuple2<List<Widget>, double> _buildDay(DateTime day) {
    ...
    final extent = HarbrBlock.calculateItemExtent(3);
    final offset = 39.30 + events.length * extent;
    final slivers = [
      SliverToBoxAdapter(
        child: HarbrHeader(text: _formatter.format(day)),
      ),
