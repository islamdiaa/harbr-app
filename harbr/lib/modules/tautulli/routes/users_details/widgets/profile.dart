...
class _State extends State<TautulliUserDetailsProfile>
  ...
  Widget _list({
    ...
    return HarbrListView(
      ...
      children: [
        const HarbrHeader(text: 'Profile'),
        _profile(user),
        const HarbrHeader(text: 'Global Stats'),
        _globalStats(watchtime),
        if (player.isNotEmpty) const HarbrHeader(text: 'Player Stats'),
  ...
  Widget _profile(TautulliUser user) {
    return HarbrTableCard(
      content: [
        HarbrTableContent(title: 'email', body: user.email),
        HarbrTableContent(
          title: 'last seen',
        ...
        HarbrTableContent(title: 'player', body: widget.user.player ?? 'None'),
        HarbrTableContent(
  ...
  List<Widget> _playerStats(List<TautulliUserPlayerStats> player) =>
      List.generate(
        player.length,
        (index) => HarbrTableCard(
          content: [
            HarbrTableContent(title: 'player', body: player[index].playerName),
            HarbrTableContent(title: 'platform', body: player[index].platform),
            HarbrTableContent(
                title: 'plays',
