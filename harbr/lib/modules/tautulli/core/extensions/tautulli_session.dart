...
extension TautulliSessionAudioExtension on TautulliSession {
  String get _language {
    if (audioLanguage?.isEmpty ?? true) return 'harbr.Unknown'.tr();
    return audioLanguage!.toTitleCase();
  }

  String get _codec {
    if (audioCodec?.isEmpty ?? true) return HarbrUI.TEXT_EMDASH;
    if (audioCodec == 'truehd') return 'TrueHD';
    return audioCodec!.toUpperCase();
  }

  String get _channelLayout {
    if (audioChannelLayout?.isEmpty ?? true) return HarbrUI.TEXT_EMDASH;
    return audioChannelLayout!.split('(')[0].toTitleCase();
  }

  String get _streamCodec {
    if (streamAudioCodec?.isEmpty ?? true) return HarbrUI.TEXT_EMDASH;
    if (streamAudioCodec == 'truehd') return 'TrueHD';
    return streamAudioCodec!.toUpperCase();
  }

  String get _streamChannelLayout {
    if (streamAudioChannelLayout?.isEmpty ?? false) return HarbrUI.TEXT_EMDASH;
    ...
    if (streamAudioDecision != null) {
      ...
      switch (streamAudioDecision) {
        case TautulliTranscodeDecision.TRANSCODE:
          return '$decision ($_language - $_codec $_channelLayout ${HarbrUI.TEXT_ARROW_RIGHT} $_streamCodec $_streamChannelLayout)';
...
extension TautulliSessionContainerExtension on TautulliSession {
  String get _container {
    if (container?.isEmpty ?? true) return 'harbr.Unknown'.tr();
    return container!.toUpperCase();
  }

  String get _streamContainer {
    if (streamContainer?.isEmpty ?? true) return 'harbr.Unknown'.tr();
  ...
  String formattedContainer() {
    if (streamContainerDecision == TautulliTranscodeDecision.TRANSCODE) {
      return '${'tautulli.Converting'.tr()} ($_container ${HarbrUI.TEXT_ARROW_RIGHT} $_streamContainer)';
    }
...
extension TautulliSessionSubtitleExtension on TautulliSession {
  String get _language {
    if (subtitleLanguage?.isEmpty ?? true) return 'harbr.Unknown'.tr();
  ...
  String get _codec {
    if ((streamSubtitleTransient ?? false) && streamSubtitleCodec != null)
      return 'tautulli.None'.tr();
    if (subtitleCodec?.isEmpty ?? true) return HarbrUI.TEXT_EMDASH;
    return subtitleCodec!.toUpperCase();
  }

  String get _streamCodec {
    if (streamSubtitleCodec?.isEmpty ?? true) return HarbrUI.TEXT_EMDASH;
  ...
  String formattedSubtitles() {
    if (subtitles ?? false) {
      ...
      switch (streamSubtitleDecision) {
        case TautulliTranscodeDecision.TRANSCODE:
          return '$decision ($_language - $_codec ${HarbrUI.TEXT_ARROW_RIGHT} $_streamCodec)';
        case TautulliTranscodeDecision.COPY:
...
extension TautulliSessionVideoExtension on TautulliSession {
  String get _codec {
    if (videoCodec?.isEmpty ?? true) return 'harbr.Unknown'.tr();
    return videoCodec!.toUpperCase();
  }

  String get _streamCodec {
    if (streamVideoCodec?.isEmpty ?? true) return 'harbr.Unknown'.tr();
    return streamVideoCodec!.toUpperCase();
  }

  String get _fullResolution {
    if (videoFullResolution?.isEmpty ?? true) return 'harbr.Unknown'.tr();
  ...
  String formattedVideo() {
    if (streamVideoDecision != null) {
      ...
      switch (streamVideoDecision) {
        case TautulliTranscodeDecision.TRANSCODE:
          return '$decision ($_codec$_hardwareDecoding $_fullResolution$_dynamicRange ${HarbrUI.TEXT_ARROW_RIGHT} $_streamCodec$_hardwareEncoding $_streamFullResolution$_streamDynamicRange)';
        case TautulliTranscodeDecision.COPY:
...
extension TautulliSessionExtension on TautulliSession {
  ...
  IconData get harbrSessionStateIcon {
    switch (this.state) {
  ...
  double get harbrSessionStateIconOffset {
    switch (this.state) {
      case TautulliSessionState.PAUSED:
        return -2.0;
      case TautulliSessionState.PLAYING:
  ...
  String get harbrETA {
    try {
      ...
      HarbrLogger().error('Failed to calculate ETA', error, stack);
      return 'harbr.Unknown'.tr();
    }
  }

  double get harbrTranscodeProgress =>
      min(1.0, max(0, this.transcodeProgress! / 100));
  double get harbrProgressPercent =>
