...
class SonarrDialogs {
  ...
  Future<Tuple2<bool, SonarrSeriesMonitorType?>> editMonitorType(
    ...
    await HarbrDialog.dialog(
      ...
      content: List.generate(
        ...
        (index) => HarbrDialog.tile(
          text: SonarrSeriesMonitorType.values[index].harbrName,
  ...
  Future<bool> deleteEpisode(BuildContext context) async {
    ...
    await HarbrDialog.dialog(
      ...
      content: [
        HarbrDialog.textContent(text: 'sonarr.DeleteEpisodeFileHint1'.tr()),
      ],
      contentPadding: HarbrDialog.textDialogContentPadding(),
    );
    return _flag;
  ...
  Future<void> showQueueStatusMessages(
    ...
    await HarbrDialog.dialog(
      ...
      cancelButtonText: 'harbr.Close'.tr(),
      contentPadding: HarbrDialog.listDialogContentPadding(),
      content: List.generate(
        messages.length,
        (index) => Padding(
            ...
            children: [
              ...
              Padding(
                padding: const EdgeInsets.symmetric(
                  vertical: HarbrUI.DEFAULT_MARGIN_SIZE / 4,
                ),
                child: Column(
                          ...
                          Expanded(
                            child: Padding(
                              padding: const EdgeInsets.only(
                                left: 32.0 + HarbrUI.ICON_SIZE,
                              ),
                              child: HarbrDialog.richText(
                                children: [
                                  TextSpan(
                                    text: messages[index]
                                        .messages!
                                        .map((s) => '${HarbrUI.TEXT_BULLET} $s')
                                        .join('\n'),
                                  ),
                                ],
                              ),
