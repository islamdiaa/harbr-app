...
@JsonSerializable()
@HiveType(typeId: 0, adapterName: 'HarbrProfileAdapter')
class HarbrProfile extends HiveObject {
  static const String DEFAULT_PROFILE = 'default';

  static HarbrProfile get current {
    final enabled = HarbrDatabase.ENABLED_PROFILE.read();
    return HarbrBox.profiles.read(enabled) ?? HarbrProfile();
  }

  static List<String> get list {
    final profiles = HarbrBox.profiles.keys.cast<String>().toList();
  ...
  Map<String, dynamic> toJson() {
    final json = _$HarbrProfileToJson(this);
    json['key'] = key.toString();
    return json;
  }

  factory HarbrProfile.fromJson(Map<String, dynamic> json) {
    return _$HarbrProfileFromJson(json);
  }

  factory HarbrProfile.clone(HarbrProfile profile) {
    return HarbrProfile.fromJson(profile.toJson().cast<String, dynamic>());
  }

  factory HarbrProfile.get(String key) {
    return HarbrBox.profiles.read(key)!;
  }

  bool isAnythingEnabled() {
    for (final module in HarbrModule.active) {
