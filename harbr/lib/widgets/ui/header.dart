import 'package:flutter/material.dart';
import 'package:harbr/core.dart';

class HarbrHeader extends StatelessWidget {
  final String? text;
  final String? subtitle;

  const HarbrHeader({
    Key? key,
    required this.text,
    this.subtitle,
  }) : super(key: key);

  Widget _headerText() {
    return Text(
      text!,
      style: const TextStyle(
        fontWeight: HarbrUI.FONT_WEIGHT_BOLD,
        fontSize: HarbrUI.FONT_SIZE_H2,
        color: Colors.white,
      ),
    );
  }

  Widget _barSeperator() {
    return Padding(
      child: Container(
        height: 2.0,
        width: HarbrUI.DEFAULT_MARGIN_SIZE * 3,
        decoration: BoxDecoration(
          borderRadius: BorderRadius.circular(HarbrUI.BORDER_RADIUS),
          color: HarbrColours.accent,
        ),
      ),
      padding: const EdgeInsets.only(
        top: HarbrUI.DEFAULT_MARGIN_SIZE / 2,
        left: 0,
        bottom: HarbrUI.DEFAULT_MARGIN_SIZE / 2,
      ),
    );
  }

  Widget _subtitle() {
    return Padding(
      child: Text(
        subtitle!,
        style: const TextStyle(
          fontSize: HarbrUI.FONT_SIZE_H4,
          color: HarbrColours.grey,
          fontWeight: FontWeight.w300,
        ),
      ),
      padding: const EdgeInsets.only(bottom: HarbrUI.DEFAULT_MARGIN_SIZE / 2),
    );
  }

  @override
  Widget build(BuildContext context) {
    return Padding(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _headerText(),
          _barSeperator(),
          if (subtitle != null) _subtitle(),
        ],
      ),
      padding: const EdgeInsets.only(
        left: HarbrUI.DEFAULT_MARGIN_SIZE,
        right: HarbrUI.DEFAULT_MARGIN_SIZE,
        top: HarbrUI.DEFAULT_MARGIN_SIZE / 2,
      ),
    );
  }
}
