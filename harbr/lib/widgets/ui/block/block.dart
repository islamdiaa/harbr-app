...
class HarbrBlock extends StatelessWidget {
  static const TITLE_HEIGHT = HarbrUI.FONT_SIZE_H2 + 4.0;
  static const SUBTITLE_HEIGHT = HarbrUI.FONT_SIZE_H3 + 4.0;
  ...
  const HarbrBlock({
    ...
    this.bodyLeadingIconsColor = HarbrColours.accent,
    this.bottom,
  ...
  static double calculateItemExtent(
    ...
    return height + HarbrUI.MARGIN_H_DEFAULT_V_HALF.vertical;
  }
  ...
  static double calculateItemHeight(
    ...
    double height = (HarbrUI.DEFAULT_MARGIN_SIZE * 2) + TITLE_HEIGHT;
      ...
      child: InkWell(
        child: Stack(
          children: [
            ...
            Opacity(
              opacity: disabled! ? HarbrUI.OPACITY_DISABLED : 1.0,
              child: Row(
                children: [
  ...
  Widget _fadeInBackground(BuildContext context, double _height) {
    ...
    final _percent = HarbrDatabase.THEME_IMAGE_BACKGROUND_OPACITY.read();
    if (_percent == 0) return const SizedBox(height: 0, width: 0);

    double _opacity = _percent / 100;
    if (disabled!) _opacity *= HarbrUI.OPACITY_DISABLED;
  ...
  Widget _poster(BuildContext context, double height) {
    double _dimension = height - HarbrUI.DEFAULT_MARGIN_SIZE;

    if (skeletonEnabled) {
      return Padding(
        padding: const EdgeInsets.only(left: HarbrUI.MARGIN_SIZE_HALF),
        child: Container(
          height: _dimension,
      ...
      return Expanded(
        child: Padding(
          child: Column(
            ...
            children: [
              Container(
                height: HarbrBlock.TITLE_HEIGHT - 4.0,
              ...
              ...List.generate(skeletonSubtitles, (_) {
                return Container(
                  height: HarbrBlock.SUBTITLE_HEIGHT - 6.0,
          ...
          padding: HarbrUI.MARGIN_DEFAULT,
        ),
      );
    }
      ...
      child: HarbrListTile(
        ...
        title: _scrollableText(
          child: HarbrText.title(
            text: title ?? HarbrUI.TEXT_EMDASH,
            color: titleColor,
